...
const DesignSystemCard = ({ designSystem }) => {
    ...
    const textBlockFont =
        typography.length > 0
            ? toSafeFontFamily(typography[0].fontFamily)
            : undefined
    ...
    const getSwatchStyle = (item) => {
        if (item.borderHex) {
            return {
                background: item.hex,
                border: `2px solid ${item.borderHex}`,
            }
        }
        if (item.opacity != null) {
            return {
                background: `color-mix(in srgb, ${item.hex} ${item.opacity}%, transparent)`,
            }
        }
            ...
            <div className={styles.dsSection}>
                ...
                <div className={styles.dsRectBlock}>
                    <div
                        ...
                        style={{
                            background:
                                gradient.bar ||
                                DEFAULT_DESIGN_SYSTEM.gradient.bar,
                        }}
                        aria-hidden
                    />
                </div>
                ...
                <div className={styles.dsRectBlock}>
                    <div
                        ...
                        style={{
                            background:
                                gradient.textBar ||
                                DEFAULT_DESIGN_SYSTEM.gradient.textBar,
                        }}
                    >
                        <span
                            className={styles.gradientBarText}
                            style={
                                textBlockFont
                                    ? { fontFamily: textBlockFont }
                                    : undefined
                            }
                        >
                ...
                <div className={styles.typographySamples}>
                    {(typography.length
                        ...
                        const fontFamily = toSafeFontFamily(typo.fontFamily)
                        return (
